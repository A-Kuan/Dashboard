...
- `src/features/dictionaries` 提供两套来源独立的字典页面、服务端分页、搜索与选项编辑。新建字典项时编码由前端即时转成大写，服务端再次规范化；已有迁移编码保持原样，避免破坏历史关联。有父子关系的选项按父项排序分组，页面突出一级项并缩进显示子项；搜索单独命中子项时仍返回上级名称作为上下文。共享 `Select` 组件在组件目录内加载自身样式，不依赖页面是否带有 `.business` 容器。
