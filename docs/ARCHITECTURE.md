...
当前范围包括顶部主导航、模块二级导航、真实数据工作台和独立报价模板模块。全站按照用户选定的 16:9 视觉图统一信息层级、轻暖页面底、冷调近白表面、蓝灰文字与亮蓝交互状态；不同业务模块保留适合自身工作流的看板、双栏、分组列表或表格结构。报价模块支持持久化管理通用车型与配件组合、填写一次性报价草稿并复制文字。导航图标使用 @tabler/icons-react。已新增 Node.js + SQLite 后端、登录、客户档案和 SKU 管理，详见下文。
...
共享布局包含 TopNavigation 和主内容区域。TopNavigation 从 config/navigation.ts 读取配置；按工作台、客户与报价、商品资料、系统设置生成顶部主导航，并为当前模块显示二级入口。它还提供当前授权范围内的功能入口搜索和账号菜单。应用名称从 VITE_APP_NAME 读取，默认 Dashboard。`main.tsx` 在各业务样式之后加载 `theme.css`，以 `#0047c3` 亮蓝、`#f5f3f0` 页面底、`#fefdfc` 表面色、`#0d1630` 正文、`#6a7993` 辅助文字和 `#dce1e7` 边框为全站色板，统一导航、页面底色、表单、表格、弹窗、抽屉和主要操作状态；各功能模块仍负责自身布局和交互样式。
